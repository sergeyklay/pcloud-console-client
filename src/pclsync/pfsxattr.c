...
static int64_t xattr_get_object_id_locked(const char *path) {
  ...
  if (!fspath) {
    log_info("path component of %s not found", path);
    return -1;
  }
  checkfile=1;
  checkfolder=1;
    ...
    if (cr) {
      ...
      if (cr->fileid>0)
        ...
        if ((row=psync_sql_fetch_rowint(res))) {
          ...
          log_warn("found temporary file for path %s but could not find task %lu", path, (unsigned long)(-cr->fileid));
          ret=-1;
        }
  ...
  if (fspath->folderid<0) {
    psync_free(fspath);
    log_info("path %s not found in temporary folder", path);
    return -1;
  }
  if (checkfolder) {
  ...
  psync_free(fspath);
  log_info("path %s not found", path);
  return -1;
}
...
int psync_fs_setxattr(const char *path, const char *name, const char *value, size_t size, int flags PFS_XATTR_IGN) {
  ...
  log_info("setting attribute %s of %s", name, path);
  LOCK_AND_LOOKUP();
...
int psync_fs_getxattr(const char *path, const char *name, char *value, size_t size PFS_XATTR_IGN) {
  ...
  if (size && value) {
    ...
    if ((row=psync_sql_fetch_row(res))) {
      ...
      if (size>=len) {
        log_debug("returning attribute %s of %s", name, path);
        memcpy(value, str, len);
        ret=len;
      }
      else {
        log_info("buffer too small for attribute %s of %s", name, path);
        ret=-ERANGE;
      }
    }
    else {
      ret=-ENOATTR;
      log_debug("attribute %s not found for %s", name, path);
    }
    psync_sql_free_result(res);
  }
  else {
    ...
    if ((row=psync_sql_fetch_rowint(res))) {
      ret=row[0];
      log_debug("returning length of attribute %s of %s = %d", name, path, ret);
    }
    else {
      ret=-ENOATTR;
      log_debug("attribute %s not found for %s", name, path);
...
int psync_fs_listxattr(const char *path, char *list, size_t size) {
  ...
  if (size && list) {
    ...
    psync_sql_free_result(res);
    log_info("returning list of attributes of %s = %d", path, ret);
  }
  else {
    psync_uint_row row;
    ...
    if ((row=psync_sql_fetch_rowint(res)))
      ...
      ret=0;
    psync_sql_free_result(res);
    log_info("returning length of attributes of %s = %d", path, ret);
  }
  psync_sql_rdunlock();
  return ret;
}
...
int psync_fs_removexattr(const char *path, const char *name) {
  ...
  psync_sql_unlock();
  if (aff) {
    log_info("attribute %s deleted for %s", name, path);
    return 0;
  }
  else {
    log_info("attribute %s not found for %s", name, path);
    return -ENOATTR;
  }
}
