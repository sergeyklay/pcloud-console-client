...
int psync_fs_update_openfile(uint64_t taskid, uint64_t writeid, psync_fileid_t newfileid, uint64_t hash, uint64_t size, time_t ctime) {
  ...
  while (tr) {
    ...
    if (d<0)
      ...
      if (fl->writeid==writeid) {
        ...
        log_info("updating fileid %ld to %lu, hash %lu size %lu", (long)fileid, (unsigned long)newfileid, (unsigned long)hash, (unsigned long)size);
        fl->fileid=newfileid;
        ...
        while (tr) {
          ...
          if (d<0) {
            ...
            log_error("found already open file %lu, should not happen", (unsigned long)newfileid);
            break;
          }
        }
        if (d<0)
        ...
        log_info("writeid of fileid %ld (%s) differs %lu!=%lu", (long)fileid, fl->currentname, (unsigned long)fl->writeid, (unsigned long)writeid);
        if (fl->newfile) {
  ...
  if ((row=psync_sql_fetch_rowint(res)) && row[0]==writeid)
    ...
    if (row)
      log_info("writeid of fileid %ld differs %lu!=%lu", (long)fileid, (unsigned long)row[0], (unsigned long)writeid);
    ret=-1;
  }
...
void psync_fs_mark_openfile_deleted(uint64_t taskid) {
  ...
  while (tr) {
    ...
    if (d<0)
      ...
      fl=psync_tree_element(tr, psync_openfile_t, tree);
      log_info("file being deleted %s is still open, marking as deleted", fl->currentname);
      psync_fs_lock_file(fl);
      fl->deleted=1;
      pthread_mutex_unlock(&fl->mutex);
...
static int psync_creat_db_to_file_stat(psync_fileid_t fileid, struct FUSE_STAT *stbuf, uint32_t flags) {
  ...
  if ((row=psync_sql_fetch_row(res)))
    ...
    log_info("fileid %lu not found in database", (unsigned long)fileid);
  psync_sql_free_result(res);
  return row?0:-1;
}
...
static int fill_stat_from_open_file(psync_fsfileid_t fileid, struct FUSE_STAT *stbuf) {
  ...
  while (tr) {
    ...
    if (d<0)
      ...
      log_info("found open file with size %lu", (unsigned long)fl->currentsize);
...
//  psync_file_t fd;
  ...
  if (fl && fl->datafile!=INVALID_HANDLE_VALUE) {
    ...
    pthread_mutex_unlock(&fl->mutex);
    if (stret)
      log_info("could not stat open file %ld", (long)cr->fileid);
    else
      log_info("got stat from open file %ld", (long)cr->fileid);
  }
  else{
    if (fl)
      pthread_mutex_unlock(&fl->mutex);
...
static int psync_creat_to_file_stat(psync_fstask_creat_t *cr, struct FUSE_STAT *stbuf, uint32_t folderflags) {
  log_info("getting stat from creat for file %s fileid %ld taskid %lu", cr->name, (long)cr->fileid, (unsigned long)cr->taskid);
  if (cr->fileid>0)
...
//  log_info("getattr %s", path);
  ...
  if (!fpath) {
    ...
    if (crr) {
      ...
      log_info("got crypto error for %s, returning %d", path, crr);
      return crr;
    }
    else{
      log_info("could not find path component of %s, returning ENOENT", path);
      return -ENOENT;
    }
  }
...
static psync_openfile_t *psync_fs_create_file(psync_fsfileid_t fileid, psync_fsfileid_t remotefileid, uint64_t size, uint64_t hash, int lock,
  ...
  while (tr) {
    ...
    if (d<0) {
      ...
      log_info("found open file %ld, refcnt %u, currentsize=%lu", (long int)fileid, (unsigned)fl->refcnt, (unsigned long)fl->currentsize);
      return fl;
    }
  }
...
int64_t psync_fs_load_interval_tree(psync_file_t fd, uint64_t size, psync_interval_tree_t **tree) {
  ...
  log_info("loading %lu intervals", (unsigned long)cnt);
  ...
  if (IS_DEBUG && *tree) {
    ...
    tr=*tree;
    log_info("loaded approx %lu intervals", (unsigned long)1<<(tr->tree.height-1));
    tr=psync_interval_tree_get_first(*tree);
    log_info("first interval from %lu to %lu", (unsigned long)tr->from, (unsigned long)tr->to);
//    while ((tr=psync_interval_tree_get_next(tr)))
//      log_info("next interval from %lu to %lu", (unsigned long)tr->from, (unsigned long)tr->to);
    tr=psync_interval_tree_get_last(*tree);
    log_info("last interval from %lu to %lu", (unsigned long)tr->from, (unsigned long)tr->to);
  }
  return cnt;
}
...
static int open_write_files(psync_openfile_t *of, int trunc) {
  ...
  int64_t fs;
  int ret;
  log_info("opening write files of %s, trunc=%d", of->currentname, trunc!=0);
  fileid=-of->fileid;
  psync_binhex(fileidhex, &fileid, sizeof(psync_fsfileid_t));
  fileidhex[sizeof(psync_fsfileid_t)]='d';
  fileidhex[sizeof(psync_fsfileid_t)+1]=0;
  ...
  if (of->datafile==INVALID_HANDLE_VALUE) {
    ...
    psync_free(filename);
    if (of->datafile==INVALID_HANDLE_VALUE) {
      log_error("could not open cache file for fileid %ld", (long)of->fileid);
      return -EIO;
    }
    ...
    log_info("data file already open");
    if (trunc)
  ...
  if (!of->newfile && of->indexfile==INVALID_HANDLE_VALUE) {
    ...
    if (of->indexfile==INVALID_HANDLE_VALUE) {
      log_error("could not open cache index file for fileid %ld", (long)of->fileid);
      return -EIO;
    }
    if (load_interval_tree(of)) {
      log_error("could not load cache file for fileid %ld to interval tree", (long)of->fileid);
      return -EIO;
    }
  }
  if (of->encrypted) {
    if (of->logfile==INVALID_HANDLE_VALUE) {
      ...
      psync_free(filename);
      if (of->logfile==INVALID_HANDLE_VALUE) {
        log_error("could not open log file for fileid %ld", (long)of->fileid);
        return -EIO;
      }
      ret=psync_fs_crypto_init_log(of);
      if (ret) {
        log_error("could not init log file for fileid %ld", (long)of->fileid);
        return ret;
      }
    }
  }
...
static int psync_fs_open(const char *path, struct fuse_file_info *fi) {
  ...
  if ((cr=psync_fstask_find_creat(folder, fpath->name, 0))) {
    if (cr->fileid>0) {
      ...
      if (row) {
        ...
        hash=row[2];
        ctime=row[3];
        log_info("opening moved regular file %lu %s size %lu hash %lu", (unsigned long)fileid, fpath->name, (unsigned long)size, (unsigned long)hash);
      }
      psync_sql_free_result(res);
      if (unlikely_log(!row)) {
        ret=-ENOENT;
      ...
      if (type==PSYNC_FS_TASK_CREAT) {
        ...
        psync_fstask_release_folder_tasks_locked(folder);
        psync_sql_unlock();
        log_info("opening new file %ld %s", (long)fileid, fpath->name);
        psync_free(fpath);
        of->newfile=1;
        ...
        log_info("opening sparse file %ld %s", (long)cr->fileid, fpath->name);
        if (fi->flags&O_TRUNC)
          size=0;
        else{
          ...
          psync_sql_free_result(res);
          if (unlikely(!row)) {
            log_warn("could not find fileid %lu with hash %lu (%ld) in filerevision", (unsigned long)fileid, (unsigned long)hash, (long)hash);
            ret=-ENOENT;
            goto ex0;
          }
        }
        ...
        log_error("trying to open file %s with id %ld but task type is %d", fpath->name, (long)cr->fileid, type);
        ret=-EIO;
        goto ex0;
      }
      if (fpath->flags&PSYNC_FOLDER_FLAG_ENCRYPTED) {
  ...
  if (!row && fpath->folderid>=0 && !psync_fstask_find_unlink(folder, fpath->name, 0)) {
    ...
    if (row) {
      ...
      hash=row[2];
      ctime=row[3];
      log_info("opening regular file %lu %s size %lu hash %lu", (unsigned long)fileid, fpath->name, (unsigned long)size, (unsigned long)hash);
    }
    psync_sql_free_result(res);
  }
  if (fi->flags&O_TRUNC || (fi->flags&O_CREAT && !row)) {
    if (fi->flags&O_TRUNC)
      log_info("truncating file %s", path);
    else
...
static int psync_fs_creat(const char *path, mode_t mode, struct fuse_file_info *fi) {
  ...
  if (psync_fs_file_exists_in_folder(folder, fpath->name)) {
    psync_fstask_release_folder_tasks_locked(folder);
    log_info("file %s already exists, processing as open", path);
...
static void psync_fs_free_openfile(psync_openfile_t *of) {
  log_info("releasing file %s", of->currentname);
  if (unlikely(of->writetimer!=PSYNC_INVALID_TIMER))
    log_error("file %s with active timer is set to free, this is not supposed to happen", of->currentname);
  if (of->deleted && of->fileid<0) {
    psync_sql_res *res;
    log_info("file %s marked for deletion, releasing cancel tasks", of->currentname);
...
static void psync_fs_upload_release_timer(void *ptr) {
  ...
  log_info("releasing file %s for upload, size=%lu, writeid=%u", ofw->of->currentname, (unsigned long)ofw->of->currentsize, (unsigned)ofw->writeid);
...
static void psync_fs_write_timer(psync_timer_t timer, void *ptr) {
  ...
  log_info("got write timer for file %s", of->currentname);
  if (of->releasedforupload)
    log_info("file seems to be already released for upload");
  else if (of->modified) {
    psync_openfile_writeid_t *ofw;
    if (unlikely(of->staticfile)) {
      log_error("file is static file, which should not generally happen");
      goto unlock_ex;
    }
    if (unlikely(of->encrypted && psync_fs_crypto_flush_file(of))) {
      log_warn("we are in timer and we failed to flush crypto file, life sux");
      goto unlock_ex;
    }
    ...
    pthread_mutex_unlock(&of->mutex);
    log_info("running separate thread to release file for upload");
    psync_run_thread1("upload release timer", psync_fs_upload_release_timer, ofw);
    return;
  }
  else
    log_info("file seems to be already uploaded");
unlock_ex:
  pthread_mutex_unlock(&of->mutex);
  psync_fs_dec_of_refcnt(of);
}
...
static int psync_fs_flush(const char *path, struct fuse_file_info *fi) {
  ...
  if (of->modified) {
    ...
    if (of->writetimer && !psync_timer_stop(of->writetimer)) {
      if (--of->refcnt==0) {
        log_error("zero refcnt in flush after canceling timer");
        assert(of->refcnt);
      }
      of->writetimer=PSYNC_INVALID_TIMER;
    }
    pthread_mutex_unlock(&of->mutex);
    log_info("releasing file %s for upload, size=%lu, writeid=%u", path, (unsigned long)of->currentsize, (unsigned)writeid);
    res=psync_sql_prep_statement("UPDATE fstask SET status=0, int1=? WHERE id=? AND status=1");
...
static int psync_read_newfile(psync_openfile_t *of, char *buf, uint64_t size, uint64_t offset) {
  ...
  pthread_mutex_unlock(&of->mutex);
  if (br==-1) {
    log_info("error reading from new file offset %lu, size %lu, error %d", (unsigned long)offset, (unsigned long)size, (int)psync_fs_err());
    br=-EIO;
  }
  return br;
}
...
static void psync_fs_inc_writeid_locked(psync_openfile_t *of) {
  if (unlikely(of->releasedforupload)) {
    ...
    if (of->releasedforupload) {
      ...
      log_info("stopping upload of file %s as new write arrived", of->currentname);
      assertw(of->fileid<0);
...
static int psync_fs_modfile_check_size_ok(psync_openfile_t *of, uint64_t size) {
  if (unlikely(of->currentsize<size)) {
    log_info("extending file %s from %lu to %lu bytes", of->currentname, (unsigned long)of->currentsize, (unsigned long)size);
...
PSYNC_NOINLINE static int psync_fs_reopen_file_for_writing(psync_openfile_t *of) {
  ...
  size_t encsymkeylen;
  int ret;
  log_info("reopening file %s for writing size %lu", of->currentname, (unsigned long)of->currentsize);
  ...
  if (size==0 || (size<=PSYNC_FS_MAX_SIZE_CONVERT_NEWFILE &&
    ...
    log_info("we have all pages of file %s, convert it to new file as they are cheaper to work with", of->currentname);
...
PSYNC_NOINLINE static int psync_fs_reopen_static_file_for_writing(psync_openfile_t *of) {
  ...
  log_info("reopening static file %s for writing size %lu, taskid %lu", of->currentname, (unsigned long)of->currentsize, (unsigned long)taskid);
...
//  log_info("free space of %s is %lld minlocal %llu", cachepath, freespc, minlocal);
  ...
  of->throttle=1;
  pthread_mutex_unlock(&of->mutex);
  log_info("free space is %lu, less than minimum %lu+%lu", (unsigned long)freespc, (unsigned long)minlocal, (unsigned long)size);
  ...
  if ((freespc<=minlocal/2 || minlocal<=PSYNC_FS_PAGE_SIZE || freespc<=size)) {
    if (psync_pagecache_free_from_read_cache(size*2)<size*2) {
      log_warn("free space is %lu, less than half of minimum %lu+%lu, returning error", (unsigned long)freespc, (unsigned long)minlocal, (unsigned long)size);
      ...
      log_info("free space is %lu, less than half of minimum %lu+%lu, but we managed to free from read cache",
    ...
    log_info("free space is %lu, less than 3/4 of minimum %lu+%lu, will try to free read cache pages",
          (unsigned long)freespc, (unsigned long)minlocal, (unsigned long)size);
  ...
  if (psync_status.uploadspeed==0) {
    if (freed || psync_pagecache_free_from_read_cache(size)>=size) {
      log_info("there is no active upload and we managed to free from cache, not throttling write");
      psync_fs_lock_file(of);
      return 1;
    }
  }
  ...
  log_info("limiting write speed to %luKb (%lub)/sec, speed multiplier %lu", (unsigned long)speed/1024, (unsigned long)speed, (unsigned long)mult);
  psync_fs_throttle(size, speed);
  log_info("continuing write");
  psync_fs_lock_file(of);
  return 1;
}
...
static int psync_fs_write(const char *path, const char *buf, size_t size, fuse_off_t offset, struct fuse_file_info *fi) {
  ...
  int ret;
  psync_fs_set_thread_name();
//  log_info("write to %s of %lu at %lu", path, (unsigned long)size, (unsigned long)offset);
  of=fh_to_openfile(fi->fh);
  psync_fs_lock_file(of);
...
retry:
  if (of->newfile) {
    ...
    if (of->encrypted)
      ...
      log_info("write of %lu bytes at offset %lu", (unsigned long)size, (unsigned long)offset);
      if (unlikely(of->staticfile)) {
...
static int psync_fs_can_rmdir(const char *path) {
  ...
  psync_sql_unlock();
  psync_free(fpath);
  log_info("can_rmdir %s=%d", path, ret);
  return ret;
}
#endif
...
static int psync_fs_can_unlink(const char *path) {
  ...
  psync_sql_unlock();
  psync_free(fpath);
  log_info("can_unlink %s=%d", path, ret);
  return ret;
}
#endif
...
static int psync_fs_rename_static_file(psync_fstask_folder_t *srcfolder, psync_fstask_creat_t *srccr, psync_fsfolderid_t to_folderid, const char *new_name) {
  ...
  if (unlikely(cr)) {
    log_info("renaming over creat of file %s in folderid %ld", new_name, (long)to_folderid);
...
static int psync_fs_rename(const char *old_path, const char *new_path) {
  ...
  int ret;
  psync_fs_set_thread_name();
  log_info("rename %s to %s", old_path, new_path);
  folder=NULL;
  psync_sql_lock();
  CHECK_LOGIN_LOCKED();
...
err_enoent:
  ...
  psync_free(fold_path);
  psync_free(fnew_path);
  log_info("returning ENOENT, folder not found");
  return -ENOENT;
}
...
static int psync_fs_chmod(const char *path, mode_t mode) {
  psync_fs_set_thread_name();
  log_info("chmod %s %u", path, (unsigned)mode);
  return 0;
}

int psync_fs_chown(const char *path, uid_t uid, gid_t gid) {
  psync_fs_set_thread_name();
  log_info("chown %s %u %u", path, (unsigned)uid, (unsigned)gid);
  return 0;
}
...
static int psync_fs_set_filetime_locked(psync_fsfileid_t fileid, const struct timespec *tv, int crtime, uint64_t current) {
  if (fileid>0)
    ...
    log_info("setting %s time of %s to %lu=%d", crtime?"creation":"modification", filename, (unsigned long)tv->tv_sec, ret);
    psync_free(filename);
    return ret?-EACCES:0;
  }
}
...
static int psync_fs_set_foldertime_locked(psync_fsfolderid_t folderid, const struct timespec *tv, int crtime, uint64_t current) {
  log_info("request to set time of folderid %ld ignored", (long)folderid);
  return 0;
}
    ...
    if ((creat=psync_fstask_find_creat(folder, name, 0))) {
      if (creat->fileid>0) {
        ...
        if ((row=psync_sql_fetch_rowint(res))) {
          ...
          psync_sql_free_result(res);
          log_warn("found creat in folderid %lu for %s with fileid %lu not present in the database",
...
static int psync_fs_setcrtime(const char *path, const struct timespec *tv) {
  psync_fs_set_thread_name();
  log_info("setcrtime %s %lu", path, tv->tv_sec);
...
static int psync_fs_utimens(const char *path, const struct timespec tv[2]) {
  psync_fs_set_thread_name();
  log_info("utimens %s %lu", path, tv[1].tv_sec);
...
static int psync_fs_ftruncate_of_locked(psync_openfile_t *of, fuse_off_t size) {
  ...
  if (of->currentsize==size) {
    log_info("not truncating as size is already %lu", (long unsigned)size);
    return 0;
  }
  psync_fs_inc_writeid_locked(of);
retry:
...
static int psync_fs_ftruncate(const char *path, fuse_off_t size, struct fuse_file_info *fi) {
  ...
  int ret;
  psync_fs_set_thread_name();
  log_info("ftruncate %s %lu", path, (unsigned long)size);
  of=fh_to_openfile(fi->fh);
  psync_fs_lock_file(of);
...
static int psync_fs_truncate(const char *path, fuse_off_t size) {
  ...
  int ret;
  psync_fs_set_thread_name();
  log_info("truncate %s %lu", path, (unsigned long)size);
...
void psync_fs_refresh() {
  ...
  pthread_mutex_unlock(&fsrefreshmutex);
  if (todo==0) {
    log_info("running cache invalidate direct");
    psync_run_thread("os cache invalidate", psync_invalidate_os_cache_noret);
  }
  else if (todo==1) {
    log_info("setting timer to invalidate cache");
...
void psync_fs_refresh_folder(psync_folderid_t folderid) {
  ...
  if (psync_invalidate_os_cache_needed())
    ...
    log_info("creating fake file %s", fpath);
...
static void psync_fs_dump_internals() {
  psync_openfile_t *of;
  log_info("dumping internal state");
  psync_sql_rdlock();
  psync_tree_for_each_element(of, openfiles, psync_openfile_t, tree)
    log_info("open file %s fileid %ld folderid %ld", of->currentname, (long)of->fileid, (long)of->currentfolder->folderid);
...
static void psync_fs_do_stop(void) {
  ...
  if (started==1) {
#if defined(P_OS_MACOSX)
    log_info("running unmount");
    unmount(psync_current_mountpoint, MNT_FORCE);
    log_info("unmount exited");
#endif
    ...
    log_info("running fuse_exit");
    fuse_exit(psync_fuse);
    started=2;
    log_info("fuse_exit exited, flushing cache");
    psync_pagecache_flush();
    log_info("cache flushed, waiting for fuse to exit");
    psync_nanotime(&ts);
    ts.tv_sec+=2;
    if (pthread_cond_timedwait(&start_cond, &start_mutex, &ts))
      log_info("timeouted waiting for fuse to exit");
    else
      log_info("waited for fuse to exit");
#if IS_DEBUG
...
static void psync_fs_wait_login() {
  log_info("waiting for online status");
  psync_wait_status(PSTATUS_TYPE_ONLINE, PSTATUS_ONLINE_ONLINE);
  log_info("waited for online status");
  psync_sql_lock();
  waitingforlogin=0;
  psync_sql_unlock();
}
...
void psync_fs_pause_until_login() {
  ...
  if (waitingforlogin==0) {
    waitingforlogin=1;
    log_info("stopping fs until login");
