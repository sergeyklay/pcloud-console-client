...
int psync_fs_crypto_init_log(psync_openfile_t *of) {
  ...
  if (unlikely(wrt!=PSYNC_CRYPTO_SECTOR_SIZE)) {
    log_error("write to log of %u bytes returned %d", (unsigned)PSYNC_CRYPTO_SECTOR_SIZE, (int)wrt);
    return -EIO;
  }
...
static int psync_fs_crypto_read_newfile_full_sector_from_log(psync_openfile_t *of, char *buf, psync_sector_inlog_t *se) {
  ...
  if (unlikely(rd!=sizeof(hdr))) {
    log_error("read from log of %u bytes returned %d", (unsigned)sizeof(hdr), (int)rd);
    return -EIO;
  }
  if (hdr.type!=PSYNC_CRYPTO_LOG_DATA) {
    log_error("bad log record type %u", (unsigned)hdr.type);
    return -EIO;
  }
  ...
  if (unlikely(rd!=hdr.length)) {
    log_error("read from log of %u bytes returned %d", (unsigned)hdr.length, (int)rd);
    return -EIO;
  }
...
//  log_info("reading first sector from offset %lu, size %u", (unsigned long)off, (unsigned)ssize);
  ...
  while (level<offsets->treelevels) {
    ...
    if (unlikely(memcmp(buff+authoff, auth, sizeof(auth)))) {
      log_warn("verify failed on level %u, sectorid %u, off %lu, ssize %u, authoff %u",
...
static int psync_fs_crypto_wait_no_extender_locked(psync_openfile_t *of) {
  ...
  while (of->extender && !of->extender->ready) {
    log_info("waiting for extender to finish");
    of->extender->waiters++;
  ...
  if (of->extender) {
    ...
    log_info("waited for extender to finish");
    return ret;
  }
  else
    return 0;
...
static int psync_fs_crypto_wait_extender_after_locked(psync_openfile_t *of, uint64_t offset) {
  while (of->extender && !of->extender->ready && of->extender->extendedto<offset) {
    log_info("waiting for extending process to reach %lu, currently at %lu", (unsigned long)offset, (unsigned long)of->extender->extendedto);
    of->extender->waiters++;
    pthread_cond_wait(&of->extender->cond, &of->mutex);
    of->extender->waiters--;
    log_info("waited extending process to reach %lu", (unsigned long)of->extender->extendedto);
  }
  if (of->extender)
...
static int psync_fs_crypto_kill_extender_locked(psync_openfile_t *of) {
  if (of->extender) {
    log_info("killing extender of file %s", of->currentname);
    of->extender->kill=1;
...
static int psync_fs_crypto_read_newfile_full_sector_from_datafile(psync_openfile_t *of, char *buf, psync_crypto_sectorid_t sectorid) {
  ...
  ssize_t rd;
  uint32_t ssize;
//  log_info("reading sector %u", sectorid);
...
//      log_info("writing level %u signatures to offset %lu size %u", level, hdr.offset, sz);
      hdr.length=sz;
      ...
      if (unlikely(wrt!=sizeof(hdr))) {
        log_error("write to log of %u bytes returned %d", (unsigned)sizeof(hdr), (int)wrt);
        return -EIO;
      }
      ...
      if (unlikely(wrt!=sz)) {
        log_error("write to log of %u bytes returned %d", (unsigned)sz, (int)wrt);
        return -EIO;
      }
    ...
    if (filesize>PSYNC_CRYPTO_AUTH_SIZE) {
      ...
      do{
        if (newsecd<=sizesecd) {
          ...
          if (unlikely(wrt!=sz)) {
            log_error("read from datafile of %u bytes returned %d at offset %u", (unsigned)sz, (int)wrt, (unsigned)off);
            return -EIO;
          }
        }
        oldsecd/=PSYNC_CRYPTO_HASH_TREE_SECTORS;
...
static int psync_fs_crypto_write_master_auth(psync_openfile_t *of, psync_crypto_auth_sector_t *autharr, psync_crypto_offsets_t *offsets) {
  ...
  if (unlikely(wrt!=sizeof(data))) {
    log_error("write to log of %u bytes returned %d", (unsigned)sizeof(data), (int)wrt);
    return -EIO;
  }
  ...
  log_info("wrote master auth to offset %lu", (unsigned long)offsets->masterauthoff);
  return 0;
}
...
PSYNC_NOINLINE static int psync_fs_crypto_check_log_hash(psync_file_t lfd, psync_crypto_master_record *mr) {
  ...
  if (unlikely(mr->hashid!=PSYNC_LOG_HASHID_FH256)) {
    log_warn("invalid hashid in log file %u", (unsigned)mr->hashid);
    return -1;
  }
  ...
  while ((rd=psync_file_pread(lfd, buff, PSYNC_CRYPTO_SECTOR_SIZE, off))!=0) {
    if (unlikely(rd==-1)) {
      log_warn("got error %d while reading from log", (int)psync_fs_err());
      return -1;
    }
  ...
  if (memcmp(chash, mr->hash, PSYNC_FAST_HASH256_LEN)) {
    log_warn("calculated hash does not match");
#if IS_DEBUG
    psync_binhex(buff, chash, PSYNC_FAST_HASH256_LEN);
    buff[PSYNC_FAST_HASH256_LEN*2]=0;
    log_info("calculated: %s", buff);
    psync_binhex(buff, mr->hash, PSYNC_FAST_HASH256_LEN);
    buff[PSYNC_FAST_HASH256_LEN*2]=0;
    log_info("expected:   %s", buff);
#endif
    return -1;
  }
  else{
    log_info("successfully checked log hash");
    return 0;
  }
}
...
static int psync_fs_crypto_process_log(psync_file_t lfd, psync_file_t dfd, psync_file_t ifd, int checkhash) {
  ...
  if (unlikely(rd!=sizeof(psync_crypto_master_record))) {
    log_warn("error reading from log file, expected to read %u got %d", (unsigned)sizeof(psync_crypto_master_record), (int)rd);
    return -1;
  }
  if (unlikely(mr->status!=PSYNC_LOG_STATUS_FINALIZED)) {
    log_warn("got log file that is not finalized, skipping");
    return -1;
  }
  if (unlikely(mr->logsize!=psync_file_size(lfd))) {
    log_warn("got log file that does not match in size, expected %u, got %d", (unsigned)mr->logsize, (int)psync_file_size(lfd));
    return -1;
  }
  if (unlikely(mr->crc!=psync_crc32c(PSYNC_CRC_INITIAL, mr, offsetof(psync_crypto_master_record, crc)))) {
    log_warn("got log file with bad master record CRC, expected %u got %u", (unsigned)mr->crc,
  ...
  if (unlikely(checkhash && psync_fs_crypto_check_log_hash(lfd, mr))) {
    log_warn("log checksum failed, skipping replay");
    return -1;
  }
  size=mr->filesize;
  off=PSYNC_CRYPTO_SECTOR_SIZE;
  ...
  while ((rd=psync_file_pread(lfd, &hdr, sizeof(hdr), off))!=0) {
    ...
    if (hdr.type==PSYNC_CRYPTO_LOG_DATA) {
      ...
      if (unlikely(hdr.offset+hdr.length>size)) {
        log_info("got record past the current end of file, this should only happen if file was truncated down, skipping record");
        off+=hdr.length;
        continue;
      }
      rd=psync_file_pread(lfd, buff, hdr.length, off);
      if (unlikely(rd!=hdr.length)) {
        log_error("error reading from log file, expected to read %u got %d", (unsigned)hdr.length, (int)rd);
        return -1;
      }
      off+=hdr.length;
      rd=psync_file_pwrite(dfd, buff, hdr.length, hdr.offset);
      if (unlikely(rd!=hdr.length)) {
        log_error("error writing to data file, expected to write %u got %d", (unsigned)hdr.length, (int)rd);
        return -1;
      }
    }
      ...
      if (++recid>=PSYNC_CRYPTO_SECTOR_SIZE/sizeof(psync_fs_index_record)) {
        ...
        if (rd!=sizeof(psync_fs_index_record)*recid) {
          log_error("error writing to index file, expected to write %u got %d", (unsigned)(sizeof(psync_fs_index_record)*recid), (int)rd);
          return -1;
        }
        ioff+=recid;
        recid=0;
      ...
      log_error("bad record type %u", (unsigned)hdr.type);
      return -1;
    }
  }
  if (recid) {
...
static void wait_before_flush(psync_openfile_t *of, uint32_t millisec) {
  log_info("waiting up to %u milliseconds before flush of %s", (unsigned)millisec, of->currentname);
...
/*static int psync_fs_flush_cache_dir() {
  ...
  log_info("flushing directory %s", path);
  ret=psync_folder_sync(path);
  if (!ret)
    log_info("flushed directory %s", path);
  return ret;
}*/
...
static int psync_fs_crypto_log_flush_and_process(psync_openfile_t *of, const char *filename, int dowaits, int locked) {
  ...
  log_info("flushing log data %s", filename);
...
//  assert(NULL=="break here to test log replay");
  ...
  if (dowaits && !locked)
    wait_before_flush(of, 2000);
  log_info("flushing data of %s", of->currentname);
  if (unlikely_log(psync_file_sync(of->datafile)))
    return -EIO;
  log_info("flushed data of %s", of->currentname);
  if (!of->newfile) {
    log_info("flushing index of %s", of->currentname);
    if (unlikely_log(psync_file_sync(of->indexfile)))
      return -EIO;
    log_info("flushed index of %s", of->currentname);
  }
  return 0;
err_eio:
  psync_file_close(fd);
...
static int psync_fs_write_interval_tree_to_log(psync_openfile_t *of) {
  ...
  while (itr) {
    if (last==ARRAY_SIZE(logs)) {
      ...
      if (wrt!=sizeof(logs)) {
        log_error("write to log of %u bytes returned %d", (unsigned)sizeof(logs), (int)wrt);
        return -EIO;
      }
  ...
  if (last) {
    ...
    if (wrt!=last) {
      log_error("write to log of %u bytes returned %d", (unsigned)(sizeof(psync_crypto_log_header)*last), (int)wrt);
      return -EIO;
    }
...
static int psync_fs_crypto_do_finalize_log(psync_openfile_t *of, int fullsync) {
  ...
  if (of->logoffset==PSYNC_CRYPTO_SECTOR_SIZE && offsets.masterauthoff+(offsets.needmasterauth?PSYNC_CRYPTO_AUTH_SIZE:0)==psync_file_size(of->datafile)) {
    log_info("skipping finalize of %s", of->currentname);
    return 0;
  }
  log_info("finalizing log of %s size %lu", of->currentname, (unsigned long)of->currentsize);
  psync_fs_crypto_offsets_by_plainsize(of->currentsize, &offsets);
  ret=psync_fs_write_auth_tree_to_log(of, &offsets);
  if (unlikely_log(ret))
    return ret;
  log_info("wrote checksums to log of %s", of->currentname);
  if (!of->newfile) {
    log_info("writing interval tree to log of %s", of->currentname);
    ret=psync_fs_write_interval_tree_to_log(of);
    if (unlikely_log(ret))
      return ret;
    log_info("wrote interval tree to log of %s", of->currentname);
  }
  ...
  if (unlikely_log(ret))
    return -EIO;
  log_info("finalized log of %s", of->currentname);
...
PSYNC_NOINLINE static void psync_fs_crypto_reset_log_to_off(psync_openfile_t *of, uint32_t off) {
  ...
  if (sz!=off)
    log_info("need to reset log from size %d to %u", (int)sz, (unsigned)off);
  if (sz==-1 || sz<off || (sz>off && (psync_file_seek(of->logfile, off, P_SEEK_SET)!=off || psync_file_truncate(of->logfile)))) {
    ...
    if (sz==-1) {
      log_error("can not stat data file of %s, can't do anything", of->currentname);
      return;
    }
    of->currentsize=psync_fs_crypto_plain_size(sz);
    log_warn("emptying log");
    ...
    if (of->logfile==INVALID_HANDLE_VALUE)
      log_warn("could not create new file %s", log);
    else if (psync_fs_crypto_init_log(of))
      log_warn("could not init log file %s", log);

  }
  else
    log_info("no need to reset log");
}
...
static int psync_fs_crypto_write_newfile_full_sector(psync_openfile_t *of, const char *buf, psync_crypto_sectorid_t sectorid, size_t size) {
  ...
  if (unlikely(wrt!=len)) {
    log_error("write to log of %u bytes returned %d", (unsigned)len, (int)wrt);
...
static int psync_fs_crypto_write_newfile_locked_nu(psync_openfile_t *of, const char *buf, uint64_t size, uint64_t offset, int checkextender) {
  ...
  assert(of->encrypted);
  assert(of->encoder);
//  log_info("write to %s size %lu, offset %lu, currentsize %lu", of->currentname, (unsigned long)size, (unsigned long)offset, (unsigned long)of->currentsize);
  if (unlikely((size+offset+PSYNC_CRYPTO_SECTOR_SIZE-1)/PSYNC_CRYPTO_SECTOR_SIZE>PSYNC_CRYPTO_MAX_SECTORID))
    return -EINVAL;
  if (unlikely(!size))
    return 0;
...
int psync_fs_crypto_read_modified_locked(psync_openfile_t *of, char *buf, uint64_t size, uint64_t offset) {
  ...
  if (rfr==1) {
    log_info("doing read at offset %lu size %lu from local changes", (unsigned long)offset, (unsigned long)size);
    return psync_fs_crypto_read_newfile_locked(of, buf, size, offset);
  }
  else if (rfr==2) {
    log_info("doing read at offset %lu size %lu from remote only", (unsigned long)offset, (unsigned long)size);
    return psync_pagecache_read_unmodified_encrypted_locked(of, buf, size, offset);
  }
  else{
    assert(rfr==3);
    log_info("doing read at offset %lu size %lu from remote and local merge", (unsigned long)offset, (unsigned long)size);
  }
  rfr=psync_pagecache_read_unmodified_encrypted_locked(of, buf, size, offset);
  if (unlikely(rfr<0)) {
    log_warn("reading from remote failed with error %d", rfr);
    return rfr;
  }
  psync_fs_lock_file(of);
  if (unlikely(offset+size>of->currentsize)) {
    log_info("file size changed during read");
    if (offset>=of->currentsize)
      return psync_fs_unlock_ret(of, 0);
    size=of->currentsize-offset;
    lastsectorid=(offset+size-1)/PSYNC_CRYPTO_SECTOR_SIZE;
...
int psync_fs_crypto_write_modified_locked_nu(psync_openfile_t *of, const char *buf, uint64_t size, uint64_t offset, int checkextender) {
  ...
  int ret;
//  log_info("off=%lu size=%lu cs=%lu ce=%d", (unsigned long)offset, (unsigned long)size, (unsigned long)of->currentsize, checkextender);
  if (unlikely((size+offset+PSYNC_CRYPTO_SECTOR_SIZE-1)/PSYNC_CRYPTO_SECTOR_SIZE>PSYNC_CRYPTO_MAX_SECTORID))
    return -EINVAL;
  if (checkextender) {
    ret=psync_fs_crypto_wait_extender_after_locked(of, offset+size);
...
retry:
  ...
  if (needtodwl) {
    ...
    do {
      ...
      if (itr->from>=eoffset)
        break;
      log_info("need to download from offset %lu size %lu to do a write at offset %lu size %lu",
    ...
    if (!icnt)
      ...
      if (unlikely(ret)) {
        psync_free(ranges);
        psync_free(tmpbuf);
        log_info("downloading of ranges failed");
        return ret;
      }
      ret=0;
      log_info("ranges downloaded");
      for (l=0; l<icnt; l++) {
        ...
        if (unlikely(itr && itr->from<ranges[l].offset+ranges[l].size && ranges[l].offset<itr->to)) {
          ...
          log_info("restarting write as range %lu to %lu downloaded and %lu to %lu local intersect", (unsigned long)ranges[l].offset,
                (unsigned long)(ranges[l].offset+ranges[l].size), (unsigned long)itr->from, (unsigned long)itr->to);
          psync_free(ranges);
          psync_free(tmpbuf);
          goto retry;
        }
        log_info("wrote %u bytes to offset %lu", (unsigned)ranges[l].size, (unsigned long)ranges[l].offset);
        bw=psync_file_pwrite(of->datafile, ranges[l].buf, ranges[l].size, ranges[l].offset);
        if (bw!=ranges[l].size) {
          log_error("write to datafile of %u bytes returned %d", (unsigned)ranges[l].size, (int)bw);
          ret=-EIO;
          break;
        }
...
static int psync_fs_crypto_ftruncate_to_zero(psync_openfile_t *of) {
  int ret;
  log_info("truncating file %s from %lu to zero", of->currentname, (unsigned long)of->currentsize);
  if (psync_file_seek(of->logfile, 0, P_SEEK_SET)!=0 || psync_file_truncate(of->logfile)) {
    log_warn("failed to truncate log file of %s", of->currentname);
    return -EIO;
  }
...
static int psync_fs_crypto_ftruncate_down(psync_openfile_t *of, uint64_t size) {
  ...
  log_info("truncating file %s from %lu down to %lu", of->currentname, (unsigned long)of->currentsize, (unsigned long)size);
  if (of->extender) {
    if (of->extender->error)
      return of->extender->error;
    if (!of->extender->ready) {
      if (of->extender->extendedto>size)
        ...
        log_info("extender is so far extended up to %lu, switching target from %lu to %lu and we are done",
...
retry:
  if (of->newfile || ((intr=psync_interval_tree_first_interval_containing_or_after(of->writeintervals, elastsectoroff)) && intr->from<=elastsectoroff)) {
    ...
    if (unlikely(of->writeid!=writeid)) {
      log_info("writeid changed, restarting");
      goto retry;
    }
  ...
  log_info("file %s truncated to %lu", of->currentname, (unsigned long)size);
  return 0;
}
...
static void psync_fs_extender_thread(void *ptr) {
  ...
  while (!ext->kill && ext->extendedto<ext->extendto) {
    ...
    ext->extendedto+=cs;
    pthread_mutex_unlock(&of->mutex);
    log_info("extender at %lu of %lu", (unsigned long)ext->extendedto, (unsigned long)ext->extendto);
    psync_yield_cpu();
    psync_fs_lock_file(of);
    if (ext->waiters) {
  ...
  if (ext->kill)
    ...
    log_info("error %d", ext->error);
  else
    log_info("finished");
  while (ext->waiters) {
    log_info("waiting for waiters to finish");
    pthread_cond_broadcast(&ext->cond);
    pthread_mutex_unlock(&of->mutex);
    psync_milisleep(1);
    psync_fs_lock_file(of);
...
static int psync_fs_crypto_run_extender(psync_openfile_t *of, uint64_t size) {
  ...
  assert(!of->extender);
  log_info("will run extender thread to extend from %lu to %lu", (unsigned long)of->currentsize, (unsigned long)size);
...
retry:
  if (of->currentsize<size) {
    log_info("truncating file %s from %lu up to %lu", of->currentname, (unsigned long)of->currentsize, (unsigned long)size);
    if (of->extender) {
      if (of->extender->ready) {
      ...
      log_info("found active extender that extended the file up to %lu of target %lu, changing target to %lu",
            (unsigned long)of->extender->extendedto, (unsigned long)of->extender->extendto, (unsigned long)size);
      of->extender->extendto=size;
      of->currentsize=size;
      return 0;
...
static void psync_fs_crypto_del_log(const char *path) {
  log_info("deleting not finalized log file %s", path);
  if (psync_file_delete(path))
    log_warn("failed to delete %s", path);
}
...
static void psync_fs_pause_task_by_name(const char *fn) {
  ...
  log_info("pausing taskid %lu", (unsigned long)taskid);
...
static void psync_fs_crypto_check_log(char *path, const char *fn) {
  ...
  if (unlikely(lfd==INVALID_HANDLE_VALUE)) {
    log_warn("could not open %s for reading, errno=%d", path, (int)psync_fs_err());
    psync_fs_pause_task_by_name(fn);
    return;
  }
  log_info("processing log file %s", path);
  plen=strlen(path)-1;
  och=path[plen];
  path[plen]='d';
  dfd=psync_file_open(path, P_O_RDWR, 0);
  if (unlikely(dfd==INVALID_HANDLE_VALUE)) {
    log_warn("could not open data file %s, errno=%d", path, (int)psync_fs_err());
  ...
  path[plen]=och;
  if (unlikely(ret)) {
    log_warn("failed to process log file %s", path);
    psync_fs_crypto_del_log(path);
  }
  else{
    psync_file_delete(path);
    log_info("processed log file %s", path);
  }
}
...
void psync_fs_crypto_check_logs() {
  const char *cachepath;
  cachepath=psync_setting_get_string(_PS(fscachepath));
  log_info("checking for unprocessed log files in %s", cachepath);
  if (psync_list_dir_fast(cachepath, psync_fs_crypto_check_file, (char *)cachepath))
    log_warn("list of %s failed", cachepath);
  log_info("log check finished");
}
