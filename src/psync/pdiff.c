...
static void delete_cached_crypto_keys() {
  ...
  if (psync_sql_affected_rows()) {
    log_info("deleted cached crypto keys");
...
static void process_modifyfolder(const binresult *entry) {
  ...
  if ((oldsync || newsync) && (oldparentfolderid!=parentfolderid || strcmp(name->str, oldname))) {
    ...
    for (i=0; i<cnt; i++) {
      ...
      if (unlikely(!row)) {
        log_error(
            "could not find local folder of folderid %lu",
            (unsigned long)folderid
        );
        psync_sql_free_result(res);
        continue;
    ...
    for (/*i is already=cnt*/; i<fres2->rows; i++) {
      ...
      log_info(
          "creating local folder %lu/%s for folderid %lu, parentfolderid %lu",
          (unsigned long)psync_get_result_cell(fres2, i, 1),
          name->str,
          (unsigned long)folderid,
          (unsigned long)parentfolderid
      );
...
static void process_createfile(const binresult *entry) {
  ...
  if (psync_is_folder_in_downloadlist(parentfolderid) && !psync_is_name_to_ignore(name->str)) {
    ...
    while ((row=psync_sql_fetch_rowint(res))) {
      ...
      if (!hasit) {
        log_info(
            "downloading file %s with hash %ld to local folder %lu",
            name->str,
            (long)hash,
            (unsigned long)row[1]
        );
...
static void send_share_notify(psync_eventtype_t eventid, const binresult *share) {
  ...
  if (!(br=psync_check_result(share, "frommail", PARAM_STR)) && !(br=psync_check_result(share, "tomail", PARAM_STR))) {
    if(!(br=psync_check_result(share, "touserid", PARAM_NUM)) &&
      ...
      log_warn(
          "Neither frommail or tomail nor business share found for eventtype %u",
          (unsigned)eventid
      );
      return;
    } else isba = 1;
  ...
  if ((br=psync_check_result(share, "foldername", PARAM_STR)) || ( br=psync_check_result(share, "sharename", PARAM_STR))) {
    ...
    psync_sql_free_result(res);
    if (!sharename) {
      log_warn("Could not find sharedfolder or sharerequest in the database.");
      return;
    }
  }
...
static void process_acceptedsharein(const binresult *entry) {
  ...
  q=psync_sql_prep_statement("REPLACE INTO sharedfolder (id, isincoming, folderid, ctime, permissions, userid, mail, name) "
                                                "VALUES (?, 1, ?, ?, ?, ?, ?, ?)");
  log_debug(
      "INSERT NORMAL SHARE IN id: %lld",
      (long long) psync_find_result(share, "shareid", PARAM_NUM)->num
  );
...
static void handle_exception(psync_socket **sock, subscribed_ids *ids, char ex) {
  if (ex=='c') {
    ...
    if (psync_select_in(&(*sock)->sock, 1, 1000)!=0) {
      log_info("got a psync_diff_wake() but no diff events in one second, closing socket");
      psync_socket_close(*sock);
      if (psync_status_get(PSTATUS_TYPE_AUTH)!=PSTATUS_AUTH_PROVIDED)
        ids->notificationid=0;
      log_info("waiting for new socket");
      *sock=get_connected_socket();
      log_info("got new socket");
  ...
  log_debug("exception handler %c", ex);
  if (ex=='r' ||
    ...
    *sock=get_connected_socket();
    ...
    if (!send_command_no_res(*sock, "nop", diffparams) ||
      ...
      *sock=get_connected_socket();
      ...
      log_info("diff socket seems to be alive");
      (*sock)->pending=1;
    }
  }
}
...
static void psync_run_analyze_if_needed() {
  if (psync_timer_time()>psync_sql_cellint("SELECT value FROM setting WHERE id='lastanalyze'", 0)+24*3600) {
    ...
    char *sql;
    size_t tablecnt, i;
    log_info("running ANALYZE on tables");
    ...
    psync_sql_run_free(res);
    log_debug("done running ANALYZE on tables");
  }
}
...
restart:
  ...
  do{
    ...
    if (entries->length) {
      ...
      log_info("processing diff with %u entries", (unsigned)entries->length);
      ids.diffid=process_entries(entries, newdiffid);
      log_info(
          "got diff with %u entries, new diffid %lu",
  ...
  log_info("initial sync finished");
