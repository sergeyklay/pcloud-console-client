    ...
    for (i=0; i<ARRAY_SIZE(settings); i++)
      if (!strcmp(name, settings[i].name)) {
        if (settings[i].type==PSYNC_TSTRING) {
          ...
          log_error("bad setting type for settingid %d (%s) expected %"P_PRI_U, i, name, settings[i].type);
      }
  }
...
psync_settingid_t psync_setting_getid(const char *name) {
  ...
  for (i=0; i<ARRAY_SIZE(settings); i++)
    if (!strcmp(name, settings[i].name))
      return i;
  log_error("setting witn name %s not found", name);
...
#define CHECK_SETTINGID_AND_TYPE(ret, stype) \
  do {\
    if (unlikely(settingid<0 || settingid>=ARRAY_SIZE(settings))) {\
      log_error("invalid settingid %d", settingid);\
      return ret;\
    }\
    if (unlikely(settings[settingid].type!=stype)) {\
      log_error("invalid setting type requested for settingid %d (%s)", settingid, settings[settingid].name);\
      return ret;\
    }\
  } while (0)\
