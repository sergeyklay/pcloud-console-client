...
static void handle_mkdir_api_error(uint64_t result, fsupload_task_t *task) {
  psync_sql_res *res;
  log_error("createfolderifnotexists returned error %u", (unsigned)result);
...
static int psync_process_task_mkdir(fsupload_task_t *task) {
  ...
  log_info("folder %lu/%s created", (unsigned long)task->folderid, task->text1);
  return 0;
}
...
static int handle_rmdir_api_error(uint64_t result, fsupload_task_t *task) {
  log_error("deletefolder returned error %u", (unsigned)result);
  psync_process_api_error(result);
  switch (result) {
...
static int psync_process_task_rmdir(fsupload_task_t *task) {
  ...
  psync_fstask_folder_deleted(task->folderid, task->id, task->text1);
  log_info("folder %lu/%s deleted", (unsigned long)task->folderid, task->text1);
  return 0;
}
...
static int large_upload_check_checksum(psync_socket *api, uint64_t uploadid, const unsigned char *filehash) {
  ...
  if (unlikely(result)) {
    log_warn("upload_info returned %lu", (long unsigned)result);
    psync_free(res);
  ...
  if (memcmp(filehash, psync_find_result(res, PSYNC_CHECKSUM, PARAM_STR)->str, PSYNC_HASH_DIGEST_HEXLEN)) {
    log_warn("upload_info returned different checksum");
    psync_free(res);
    psync_apipool_release(api);
    return -1;
  }
...
static int handle_upload_api_error(uint64_t result, fsupload_task_t *task) {
  log_error("uploadfile returned error %u", (unsigned)result);
...
static int save_meta(const binresult *meta, psync_folderid_t folderid, const char *name, uint64_t taskid, uint64_t writeid, int newfile,
  ...
  if (psync_fs_update_openfile(taskid, writeid, fileid, hash, size, psync_find_result(meta, "created", PARAM_NUM)->num)) {
    ...
    if (row && row[0]==11) {
      ...
      deleted=1;
      log_info("detected cancel of upload of %s too late, processing normally, delete will come", name);
    }
    else {
      psync_sql_free_result(sql);
      psync_sql_rollback_transaction();
      log_info("upload of %s task %lu cancelled due to writeid mismatch", name, (unsigned long)taskid);
      return -1;
    }
  }
  if (newfile) {
  ...
  if (!psync_sql_affected_rows()) {
    log_error("upload of %s cancelled due to writeid mismatch, writeid %lu, psync_fs_update_openfile should have catched that", name, (long unsigned)writeid);
    psync_sql_rollback_transaction();
    return -1;
  }
  psync_sql_commit_transaction();
  log_info("file %lu/%s uploaded (mtime=%lu, size=%lu)", (unsigned long)folderid, name,
...
static int large_upload_save(psync_socket *api, uint64_t uploadid, psync_folderid_t folderid, const char *name,
  ...
  if (psync_stat(filepath, &st)) {
    log_warn("could not stat file %s", filepath);
    psync_apipool_release(api);
    return -1;
  }
  if (key) {
  ...
  result=psync_find_result(res, "result", PARAM_NUM)->num;
  if (unlikely(result)) {
    log_warn("upload_save returned %lu", (long unsigned)result);
    psync_free(res);
    handle_upload_api_error_taskid(result, taskid);
    return -1;
  }
  log_info("sent mtime=%lu", (unsigned long)psync_stat_mtime(&st));
...
static void perm_fail_upload_task(uint64_t taskid) {
  psync_sql_res *sql;
  log_warn("failed task %lu", (unsigned long)taskid);
  psync_sql_start_transaction();
  sql=psync_sql_prep_statement("DELETE FROM fstaskdepend WHERE dependfstaskid=?");
  psync_sql_bind_uint(sql, 1, taskid);
  psync_sql_run_free(sql);
...
static int copy_file_if_exists(psync_socket *api, const char *filename, const unsigned char *hashhex, uint64_t fsize, psync_folderid_t folderid, const char *name,
  ...
  if (ret==1) {
    log_info("file %lu/%s copied to %lu/%s instead of uploading due to matching checksum",
...
static int large_upload_creat(uint64_t taskid, psync_folderid_t folderid, const char *name, const char *filename,
  ...
  log_info("uploading %s as %lu/%s (uploadid=%lu)", filename, (unsigned long)folderid, name, (unsigned long)uploadid);
  asize=0;
  if (uploadid) {
  ...
  if (ret) {
    ...
    log_warn("could not open local file %s, skipping task", filename);
    return 0;
  }
  ...
  if (unlikely_log(fd==INVALID_HANDLE_VALUE))
    goto ret0;
  if (usize) {
    log_info("resuming from offset %lu", (unsigned long)usize);
  ...
  while (usize<fsize) {
    if (unlikely(stop_current_upload)) {
      log_info("got stop for file %s", name);
      goto err2;
    }
  ...
  psync_free(res);
  if (result) {
    log_warn("upload_write returned error %lu", (long unsigned)result);
    psync_process_api_error(result);
  ...
  if (psync_fs_get_file_writeid(taskid)!=writeid) {
    log_info("%s changed while uploading as %lu/%s", filename, (unsigned long)folderid, name);
    psync_apipool_release(api);
    goto errs;
  }
  if (asize) {
...
static int upload_modify_send_copy_from(psync_socket *api, psync_uploadid_t uploadid, uint64_t offset, uint64_t length,
  ...
  log_info("copying %lu bytes from fileid %lu hash %lu at offset %lu", (unsigned long)length, (unsigned long)fileid, (unsigned long) hash, (unsigned long)offset);
...
static int upload_modify_send_local(psync_socket *api, psync_uploadid_t uploadid, uint64_t offset, uint64_t length, psync_file_t fd, uint64_t *upl) {
  ...
  size_t rd;
  ssize_t rrd;
  log_info("uploading %lu byte from local file at offset %lu", (unsigned long)length, (unsigned long)offset);
...
static int upload_modify_read_req(psync_socket *api) {
  ...
  psync_free(res);
  if (result) {
    log_warn("got %lu from upload_writefromfile or upload_write", (unsigned long)result);
    psync_process_api_error(result);
...
int upload_modify(uint64_t taskid, psync_folderid_t folderid, const char *name, const char *filename, const char *indexname, psync_fileid_t fileid,
  ...
  psync_fs_err_t err;
  int ret;
  log_info("uploading modified file %s writeid %lu as %lu/%s", filename, (unsigned long)writeid, (unsigned long)folderid, name);
  asize=0;
  fd=psync_file_open(indexname, P_O_RDONLY, 0);
  if (unlikely(fd==INVALID_HANDLE_VALUE)) {
    err=psync_fs_err();
    log_warn("can not open %s", indexname);
    if (err==P_NOENT) {
      perm_fail_upload_task(taskid);
      return 0;
    }
  ...
  if (unlikely(fd==INVALID_HANDLE_VALUE)) {
    err=psync_fs_err();
    log_warn("can not open %s", filename);
    psync_apipool_release(api);
    psync_interval_tree_free(tree);
    if (err==P_NOENT) {
      perm_fail_upload_task(taskid);
  ...
  if (unlikely_log(fsize==-1))
    goto err3;
  log_info("file size=%lu", (unsigned long)fsize);
  coff=0;
  reqs=0;
  ...
  while (coff<fsize) {
    ...
    if (cinterval) {
      if (cinterval->from>coff) {
        ...
        log_error("broken interval tree");
        break;
      }
    }
    else {
...
static void large_upload() {
  ...
  while (1) {
    ...
    if (type==PSYNC_FS_TASK_CREAT)
      ...
      ret=0;
      log_error("wrong type %lu for task %lu", (unsigned long)type, (unsigned long)taskid);
...
static int psync_send_task_creat(psync_socket *api, fsupload_task_t *task) {
  if (api) {
    ...
    if (size>PSYNC_FS_DIRECT_UPLOAD_LIMIT) {
      psync_file_close(fd);
      log_info("defering upload of %lu/%s due to size of %lu", (unsigned long)task->folderid, task->text1, (unsigned long)size);
      return -2;
    }
    else {
      log_info("uploading file %lu/%s pipelined due to size of %lu", (unsigned long)task->folderid, task->text1, (unsigned long)size);
...
static int psync_process_task_creat(fsupload_task_t *task) {
  ...
  if (psync_fs_update_openfile(task->id, task->int1, fileid, hash, psync_find_result(meta, "size", PARAM_NUM)->num, psync_find_result(meta, "created", PARAM_NUM)->num)) {
    log_info("file %lu/%s changed while uploading, failing task", (unsigned long)task->folderid, task->text1);
    return -1;
  }
  psync_ops_create_file_in_db(meta);
  ...
  task->int2=fileid;
  log_info("file %lu/%s uploaded", (unsigned long)task->folderid, task->text1);
  psync_sql_commit_transaction();
...
static int psync_send_task_unlink(psync_socket *api, fsupload_task_t *task) {
  ...
  if (!api) {
    log_info("cancelling task %lu", (unsigned long)task->id);
    return 0;
  }
...
static int handle_unlink_api_error(uint64_t result, fsupload_task_t *task) {
  log_error("deletefile returned error %u for fileid %lu", (unsigned)result, (unsigned long)task->fileid);
  psync_process_api_error(result);
  switch (result) {
...
static int psync_process_task_unlink(fsupload_task_t *task) {
  ...
  psync_fstask_file_deleted(task->folderid, task->id, task->text1);
  log_info("file %lu/%s deleted", (unsigned long)task->folderid, task->text1);
  return 0;
}
...
static int handle_rename_file_api_error(uint64_t result, fsupload_task_t *task) {
  log_error("renamefile returned error %u", (unsigned)result);
  psync_process_api_error(result);
  switch (result) {
...
static int psync_process_task_rename_file(fsupload_task_t *task) {
  ...
  psync_ops_update_file_in_db(meta);
  psync_fstask_file_renamed(task->folderid, task->id, task->text1, task->int1);
  log_info("file %lu/%s renamed", (unsigned long)task->folderid, task->text1);
  return 0;
}
...
static void change_folder_name(fsupload_task_t *task) {
  ...
  psync_sql_run_free(res);
  log_info("changed target name of task %lu from %s to %s", (unsigned long)task->id, task->text1, nn);
  psync_free(nn);
}

static int handle_rename_folder_api_error(uint64_t result, fsupload_task_t *task) {
  log_error("renamefolder returned error %u parentfolderid=%lu name=%s", (unsigned)result, (unsigned long)task->folderid, task->text1);
  psync_process_api_error(result);
  switch (result) {
...
static int psync_process_task_rename_folder(fsupload_task_t *task) {
  ...
  psync_fstask_folder_renamed(task->folderid, task->id, task->text1, task->int1);
  log_info("folder %lu/%s renamed", (unsigned long)task->folderid, task->text1);
  return 0;
}
...
static int psync_cancel_task_creat(fsupload_task_t *task) {
  ...
  psync_sql_free_result(res);
  if (fileid) {
    log_info("cancelled creat task %lu for file %s, changed to fileid %lu", (unsigned long)task->id, task->text1, (unsigned long)fileid);
...
static int psync_cancel_task_rename_file(fsupload_task_t *task) {
  psync_fstask_file_renamed(task->folderid, task->id, task->text1, task->int1);
  log_info("cancelled rename task %lu (from task %lu) for file %s", (unsigned long)task->id, (unsigned long)task->int1, task->text1);
  return 0;
}
...
static int psync_cancel_task_modify(fsupload_task_t *task) {
  ...
  psync_sql_bind_int(res, 2, -(psync_fsfileid_t)task->id);
  psync_sql_run_free(res);
  log_info("cancelled modify task %lu for file %s, changed to fileid %lu", (unsigned long)task->id, task->text1, (unsigned long)task->fileid);
  return 0;
}
...
//  psync_uint_row row;
  ...
  if (task->int2) {
    log_info("requested cancel of delete of a modified file, deleting fileid %ld instead for file %s", (long)task->int2, task->text1);
...
static int psync_cancel_task_unlink_set_rev(fsupload_task_t *task) {
  psync_sql_res *res;
  if (task->int2) {
    log_info("converting cancelled unlink_set_rev task %lu to a normal unlink task for file %s", (unsigned long)task->id, task->text1);
  ...
  psync_list_for_each_element (task, tasks, fsupload_task_t, list) {
    ...
    if (task->status==11) {
      ...
      if (psync_process_task_func[task->type](task))
        log_warn("processing task %lu of type %lu failed", (unsigned long)task->id, (unsigned long)task->type);
      else {
...
static void psync_fsupload_run_tasks(psync_list *tasks) {
  ...
  psync_list_for_each_element (task, tasks, fsupload_task_t, list) {
    ...
    if (!task->type || task->type>=ARRAY_SIZE(psync_send_task_func)) {
      log_error("bad task type %lu", (unsigned long)task->type);
      continue;
    }
    if (task->status==11) {
...
//    log_info("will process taskid %lu", (unsigned long)task->id);
  }
