...
static void psync_p2p_check(const packet_check *packet) {
  ...
  log_info("replying with %u to a check from %s, looking for %."NTO_STR(PSYNC_HASH_DIGEST_HEXLEN)"s", (unsigned int)resp.type, p2p_get_peer_address(), hashhex);
  if (files_serving)
  ...
  if (!sendto(udpsock, (const char *)&resp, sizeof(resp), 0, (const struct sockaddr *)&paddr, paddrlen))
    log_warn("sendto to %s failed", p2p_get_peer_address());
}
...
static void psync_p2p_process_packet(const char *packet, size_t plen) {
  ...
  if (type>=ARRAY_SIZE(min_packet_size) || min_packet_size[type]>plen)
    return;
  log_info("got %u packet from %s", (unsigned int)type, p2p_get_peer_address());
  switch (type) {
    case P2P_WAKE:
      break;
    case P2P_CHECK:
      psync_p2p_check((packet_check *)packet);
      log_info("processed P2P packed");
      break;
    default:
      log_error("handler for packet type %u not implemented", (unsigned)type);
      break;
  }
}
...
static void psync_p2p_tcphandler(void *ptr) {
  ...
  if (!localfileid) {
    log_warn("got request for file that we do not have");
    goto err0;
  }
  ...
  log_info("sending file %s to peer", localpath);
  psync_free(localpath);
  if (fd==INVALID_HANDLE_VALUE) {
    log_warn("could not open local file %lu", (unsigned long)localfileid);
    goto err0;
  }
  ...
  psync_crypto_aes256_ctr_encoder_decoder_free(encoder);
  psync_file_close(fd);
  log_info("file sent successfuly");
err0:
...
static int psync_p2p_check_rsa() {
  ...
  if (psync_rsa_private==PSYNC_INVALID_RSA) {
    ...
    log_info("generating %ubit RSA key", PSYNC_P2P_RSA_SIZE);
...
static int psync_p2p_download(psync_socket_t sock, psync_fileid_t fileid, const unsigned char *filehashhex, uint64_t fsize, const char *filename) {
  ...
  if (enctype!=P2P_ENCTYPE_RSA_AES) {
    log_error("unknown encryption type %u", (unsigned)enctype);
    return PSYNC_NET_PERMFAIL;
  }
  if (keylen>PSYNC_P2P_RSA_SIZE/8*2) { /* PSYNC_P2P_RSA_SIZE/8 is enough actually */
    log_error("too long key - %u bytes", (unsigned)keylen);
    return PSYNC_NET_PERMFAIL;
  }
  ...
  psync_binhex(hashhex, hashbin, PSYNC_HASH_DIGEST_LEN);
  log_info("downloaded file %s from peer", filename);
  if (memcmp(hashhex, filehashhex, PSYNC_HASH_DIGEST_HEXLEN)) {
    ...
    log_warn("got bad checksum for file %s", filename);
    return PSYNC_NET_PERMFAIL;
  }
  else
    return PSYNC_NET_OK;
...
int psync_p2p_check_download(psync_fileid_t fileid, const unsigned char *filehashhex, uint64_t fsize, const char *filename) {
  ...
  if (!psync_setting_get_bool(_PS(p2psync)))
    return PSYNC_NET_PERMFAIL;
  log_info("sending P2P_CHECK for file with hash %."NTO_STR(PSYNC_HASH_DIGEST_HEXLEN)"s", filehashhex);
  pct1.type=P2P_CHECK;
  ...
  for (i=0; i<il->interfacecnt; i++) {
    ...
    if (unlikely(sock==INVALID_SOCKET)) {
      log_info("could not create a socket for address family %u", (unsigned)il->interfaces[i].address.ss_family);
      continue;
    }
  ...
  for (i=0; i<il->interfacecnt; i++)
    if (sockets[i]!=INVALID_SOCKET && FD_ISSET(sockets[i], &rfds)) {
      ...
      if (!memcmp(pct1.rand, resp.rand, sizeof(resp.rand))) {
        log_warn("clients are supposed to generate random data, not to reuse mine");
        continue;
      }
      memcpy(hashsource, filehashhex, PSYNC_HASH_DIGEST_HEXLEN);
      memcpy(hashsource+PSYNC_HASH_DIGEST_HEXLEN, resp.rand, sizeof(resp.rand));
  ...
  if (addr.sin6_family==AF_INET6) {
    ...
    log_error("unknown address family %u", (unsigned)addr.sin6_family);
    goto err_perm2;
  }
  if (unlikely_log(sock==INVALID_SOCKET))
    goto err_perm3;
  if (unlikely(connect(sock, (struct sockaddr *)&addr, slen)==SOCKET_ERROR)) {
    log_warn("could not connect to %s port %u", p2p_get_address(&addr), (unsigned)resp.port);
    goto err_perm3;
  }
  ...
  if (socket_write_all(sock, &pct2, sizeof(pct2)) ||
    ...
    log_warn("writing to socket failed");
    goto err_temp3;
  }
  psync_free(token);
