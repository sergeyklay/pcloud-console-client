...
void *psync_cache_get(const char *key) {
  ...
  if (IS_DEBUG && !strcmp(psync_thread_name, "timer"))
    log_error("trying get key %s from the timer thread, this may (and eventually will) lead to a deadlock, "
  ...
  if (maxkeys) {
    ...
    psync_list_for_each_element (he2, lst, hash_element, list)
      if (unlikely(he2->hash==h && !strcmp(key, he2->key) && ++l==maxkeys)) {
        ...
        psync_free(he);
        freefunc(ptr);
        log_debug("not adding key %s to cache as there already %u elements present", key, (unsigned int)maxkeys);
        return;
      }
  }
